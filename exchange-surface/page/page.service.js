import { getTimezoneOffset } from 'drapcode-constant';
import {
  capitalize,
  lowerCase,
  upperCase,
...
import { count, htmlRegex } from '../utils/util';
...
import { replaceSlashWithUnderscore, replaceUnderscoreWithSlash, loadPage } from 'drapcode-utility';
import {
  extractFirstSubTenantIdFromUserAndTenantId,
  extractUserSettingFromUserAndTenant,
} from '../apiService/multiTenant.service';
const { JSDOM } = jsdom;
...
const jsDomUpdatePageContent = async (
  ...
  if (itemData) {
    ...
    loadLoggedInSubTenantDataIntoElements(window, user, req, s3Url);
...
const loadLoggedInSubTenantDataIntoElements = async (window, user, req, s3Url) => {
  const { document } = window;
  ...
  if (sessionAttributes) {
    ...
    if (
      ...
      sessionAttributes.forEach((element) => {
        ...
        loadDataIntoElements(element, fieldName, loggedInSubTenant, req, s3Url);
      });
    }
  }
};
