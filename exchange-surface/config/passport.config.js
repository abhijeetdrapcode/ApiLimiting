...
const getOAuthOptionsFromPlugin = async (req, res) => {
  ...
  let pluginOptions = {};
  const oAuth2Plugin = await findOneInstalledPlugin(projectId, pluginCode.OAUTH_2);
  if (!oAuth2Plugin) {
...
export const configureFacebookPassport = async (req, res, next) => {
  ...
  const facebookPlugin = await findOneInstalledPlugin(projectId, pluginCode.FACEBOOK_LOGIN);
  if (!facebookPlugin) {
...
export const configureTwitterPassport = async (req, res, next) => {
  ...
  const twitterPlugin = await findOneInstalledPlugin(projectId, pluginCodeTWITTER_LOGIN);
  if (!twitterPlugin) {
...
export const configureDocusignPassport = async (req, res, next) => {
  ...
  const docusignPlugin = await findOneInstalledPlugin(projectId, pluginCode.DOCUSIGN);
  if (!docusignPlugin) {
