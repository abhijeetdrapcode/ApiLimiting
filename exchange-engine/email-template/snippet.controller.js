...
const getDynamicPDFContentHtml = async (config, cleanHTML = false, style = '') => {
  ...
  if (cleanHTML) {
    pdfContentHtml = cleanForPuppeteer(pdfContentHtml);
    pdfContentHtml = `
    <style>
    ${style}
    </style>
...
export const downloadPDFTemplateContent = async (req, res, next) => {
  try {
    ...
    const pdfStyle = pdfSnippet.customStyle;
    ...
    const pdfContentHtml = await getDynamicPDFContentHtml(pdfContentConfig);
