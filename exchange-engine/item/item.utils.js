...
export const convertToFindOneAndUpdateQuery = (query, currentDoc) => {
  const updateObj = {};
  if (query.$set) {
    const setFields = { ...query.$set };
    if (setFields.version === 'INC') {
      let currentVersion = currentDoc?.version;
      if ([undefined, null, ''].includes(currentVersion) || typeof currentVersion !== 'number') {
        currentVersion = 0;
      } else if (typeof currentVersion === 'string') {
        const parsed = parseInt(currentVersion, 10);
        currentVersion = isNaN(parsed) ? 0 : parsed;
      }
  ...
  if (query.$unset) updateObj.$unset = query.$unset;
  return updateObj;
};

export const checkDerivedFieldMapping = async (
  derivedFieldMappingId,
  derivedFieldMapping,
  items,
  user,
) => {
  const wasArray = Array.isArray(items);
  items = wasArray ? items : [items];
  const selectedDerivedFieldMapping = derivedFieldMapping.find(
    (df) => df.uuid === derivedFieldMappingId,
  );
  const mapping = selectedDerivedFieldMapping
    ? selectedDerivedFieldMapping.derivedFieldMapping
    : [];
  if (!mapping || mapping.length === 0) {
    return wasArray ? items : items[0];
  }
  const updatedItems = items.map((item) => {
    let newItem = { ...item };
    mapping.forEach(({ derivedField, fieldName }) => {
      const result = prepareFunction(derivedField, newItem, user);
      if (result && typeof result === 'object') {
        newItem = { ...newItem, ...result };
      }
      newItem[fieldName] = result;
    });
    return newItem;
  });
  return wasArray ? updatedItems : updatedItems[0];
};
